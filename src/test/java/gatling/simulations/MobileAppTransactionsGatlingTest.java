...
/**
 * Performance test for the MobileAppTransactions entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class MobileAppTransactionsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all mobileAppTransactions")
                    .get("/services/mobileapptransaction/api/mobile-app-transactions")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mobileAppTransactions")
                        .post("/services/mobileapptransaction/api/mobile-app-transactions")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"channel\": \"SAMPLE_TEXT\"" +
                                ", \"channelIp\": \"SAMPLE_TEXT\"" +
                                ", \"channelReference\": \"SAMPLE_TEXT\"" +
                                ", \"channelTimestamp\": \"SAMPLE_TEXT\"" +
                                ", \"clientId\": \"SAMPLE_TEXT\"" +
                                ", \"createdAt\": \"SAMPLE_TEXT\"" +
                                ", \"debitAccount\": \"SAMPLE_TEXT\"" +
                                ", \"direction\": \"SAMPLE_TEXT\"" +
                                ", \"errorDescription\": \"SAMPLE_TEXT\"" +
                                ", \"geolocation\": \"SAMPLE_TEXT\"" +
                                ", \"hostCode\": \"SAMPLE_TEXT\"" +
                                ...
                                ", \"userAgentVersion\": \"SAMPLE_TEXT\"" +
                                ", \"amount\": \"SAMPLE_TEXT\"" +
                                ", \"chargeamount\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mobileAppTransactions_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created mobileAppTransactions")
                            .get("/services/mobileapptransaction${new_mobileAppTransactions_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created mobileAppTransactions")
                        .delete("/services/mobileapptransaction${new_mobileAppTransactions_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MobileAppTransactions entity").exec(scn);
